                ...
                <form>
                    <div class="row">
                        <div class="input-group col-sm-4 col-md-3">
                            ...
                            <input name="id" type="text" class="form-control" placeholder="Offer ID"
                                value="{{ request('id') }}">
                        </div>
                        ...
                        <div class="input-group col-sm-4 col-md-3">
                            ...
                            <input name="offerer" type="text" class="form-control"
                                placeholder="User email, mobile or username" value="{{ request('offerer') }}">
                        </div>

                        <div class="col-sm-4 col-md-3">
                            <select name="status" class="form-control text-capitalize">
                                <option value="">-- Status --</option>
                                @foreach(\Bitoff\Mantis\Application\Models\Offer::isActive() as $status)
                                <option value="{{(int)$status}}" {{ (request('status') !== null && request('status') == $status) ? 'selected' : '' }}>
                        ...
                        <div class="col-md-3">
                            <select class="form-control" name="isBuy">
                                ...
                                @foreach(\Bitoff\Mantis\Application\Models\Offer::isBuy() as $isBuy)
                                <option value="{{(int)$isBuy}}" {{ (request('isBuy') !== null && request('isBuy') == $isBuy) ? 'selected' : '' }}>
                                    {{ $isBuy ? 'Buy' : 'Sell' }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-md-1">
                            <a href="{{ route('mantis.offers.index') }}" class="btn btn-danger btn-block">Reset</a>
                        </div>
                    </div>
                </form>
            </div>
